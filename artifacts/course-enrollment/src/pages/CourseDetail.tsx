...
import { Clock, BarChart, Globe, IndianRupee, ShieldCheck, PlayCircle, FileText } from 'lucide-react';
...
export default function CourseDetail() {
  ...
  const isEnrolled = enrollments?.some(e => e.courseId === courseId) || user?.role === 'admin';

  if (isLoading) {
    return <div className="min-h-[80vh] flex items-center justify-center"><Spinner className="w-12 h-12" /></div>;
  }
    ...
    <div className="w-full bg-background min-h-screen pb-24">
          ...
          <div className="grid lg:grid-cols-[1fr_400px] gap-12 items-center">
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
              ...
              <div className="flex flex-wrap gap-6 text-sm font-medium text-foreground">
                <div className="flex items-center gap-2"><Clock className="w-5 h-5 text-muted-foreground"/> Self-paced</div>
                <div className="flex items-center gap-2"><BarChart className="w-5 h-5 text-muted-foreground"/> All levels</div>
                <div className="flex items-center gap-2"><Globe className="w-5 h-5 text-muted-foreground"/> English</div>
              </div>
            </motion.div>
            ...
            >
              <Card className="p-1 overflow-hidden sticky top-28 shadow-2xl shadow-black/10 border-2">
                <div className="aspect-video relative rounded-xl overflow-hidden bg-muted mb-6 group cursor-pointer">
                  <img src={course.thumbnailUrl || fallbackImage} alt={course.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                    ...
                    <>
                      <div className="flex items-end gap-2 mb-6">
                        <span className="text-4xl font-display font-bold text-foreground flex items-center">
                          <IndianRupee className="w-8 h-8 mr-1 text-primary" />{course.price}
                        </span>
                      </div>
                      <Button size="lg" className="w-full text-lg h-14 shadow-primary/30" onClick={() => user ? setIsPaymentModalOpen(true) : window.location.href = '/auth'}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="max-w-3xl">
          <h2 className="text-3xl font-display font-bold text-foreground mb-8">Course Content</h2>
          
          {isEnrolled ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="prose prose-lg dark:prose-invert max-w-none text-foreground/90 leading-relaxed bg-card p-8 rounded-3xl border border-border/50 shadow-sm">
            ...
            <div className="bg-card border border-border/50 rounded-3xl p-8 shadow-sm">
              <div className="relative">
                ...
                <div className="absolute inset-0 bg-gradient-to-b from-transparent via-card/80 to-card flex flex-col items-center justify-end pb-8">
                  ...
                  <h3 className="text-xl font-bold text-foreground mb-2">Content Locked</h3>
                  <p className="text-muted-foreground mb-6">Enroll in this course to access the full learning materials.</p>
                  <Button onClick={() => user ? setIsPaymentModalOpen(true) : window.location.href = '/auth'} className="shadow-primary/20">
                    Enroll to unlock
                  </Button>
                </div>
              </div>
