      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12 space-y-6">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-4xl md:text-5xl font-display font-extrabold text-foreground mb-4">Explore All Courses</h1>
            <p className="text-lg text-muted-foreground max-w-3xl">
              Discover the perfect course to level up your skills. From programming to design, we have something for everyone.
            </p>
          </motion.div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3, 4, 5, 6].map(i => (
              <div key={i} className="h-[420px] bg-card/50 border border-border/50 animate-pulse rounded-2xl" />
            ))}
          </div>
          ...
          <div className="text-center py-32 bg-card rounded-3xl border border-border/50 shadow-sm">
            ...
            <p className="text-muted-foreground text-lg">Try adjusting your search terms to find what you're looking for.</p>
          </div>
        )}
      </div>
    </div>
